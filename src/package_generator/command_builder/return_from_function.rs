...
pub fn return_command_builder(action: &ReturnAction, defined_data: &Vec<DataDeclarator>, metadata: &PackageMetadata) -> RelocatableCommandList {
    if action.value.is_some() {
        ...
        let expr_eval_command = build_expression_evaluation_command(&action.value.clone().unwrap(), defined_data, metadata);

        let mut result = RelocatableCommandList::new_no_relocation(expr_eval_command);
