...
pub fn build_assignment_command(action: &AssignmentAction, defined_data: &Vec<DataDeclaration>, metadata: &PackageMetadata) -> Vec<u8> {
    ...
    result.push(combine_command(RootCommand::Stack.to_opcode(), StackCommand::PopToObject.to_opcode()));
    // Push GDF(visit src/shared/command_map.rs)
    result.push(combine_command(u8::from(target_data.is_global), PLACE_HOLDER));
    // Push slot id
