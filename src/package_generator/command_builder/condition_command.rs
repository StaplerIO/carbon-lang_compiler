...
pub fn if_command_builder(action: &IfAction, defined_functions: &Vec<FunctionDescriptor>, defined_data: &Vec<DataDeclarator>, metadata: &PackageMetadata) -> RelocatableCommandList {
    ...
    // Build IfBlock
    result.combine(condition_block_builder(&action.if_block, ConditionBlockType::IfBlock, remaining_domain_count, defined_functions, defined_data, metadata));
    ...
    for elif_block in &action.elif_collection {
        result.combine(condition_block_builder(elif_block, ConditionBlockType::ElifBlock, remaining_domain_count, defined_functions, defined_data, metadata));
        remaining_domain_count -= 1;
        result.combine(direct_jump_command_builder(RelocationTargetType::BreakDomain(remaining_domain_count), metadata));
    }
    ...
    if action.else_action.is_some() {
        ...
        result.combine(action_block_builder(&action.else_action.as_ref().unwrap(), true, defined_functions, defined_data, metadata));
...
pub fn while_command_builder(action: &WhileBlock, defined_functions: &Vec<FunctionDescriptor>, defined_data: &Vec<DataDeclarator>, metadata: &PackageMetadata) -> RelocatableCommandList {
    let mut result = condition_block_builder(action, ConditionBlockType::WhileBlock, 0, defined_functions, defined_data, metadata);
