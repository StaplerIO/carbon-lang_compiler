...
use crate::shared::ast::action::{IfAction, ActionBlock, ConditionBlock};
...
pub fn if_block_builder(tokens: Vec<DecoratedToken>) -> (Option<IfAction>, isize) {
                    ...
                    if expression_zone.len() >= 1 {
                        ...
                        if tokens[expression_zone.len() + 2].token_type == DecoratedTokenType::Container {
                            if tokens[expression_zone.len() + 2].container.unwrap() == ContainerType::Brace {
                                ...
                                return (Option::from(result), current_index as isize);
                            }
                        }
                    }
                }
